-- Add visibility column to check_ins table for discovery privacy control.
-- Values: 'public' (visible to all), 'friends_only' (future Phase 3), 'private' (hidden from roster).
-- Default 'public' ensures backward compatibility — all existing check-ins remain visible.

ALTER TABLE check_ins ADD COLUMN visibility TEXT NOT NULL DEFAULT 'public';

ALTER TABLE check_ins ADD CONSTRAINT check_ins_visibility_check
    CHECK (visibility IN ('public', 'friends_only', 'private'));

-- Update check_in_to_place_v2 to accept and persist visibility parameter.
-- Adds p_visibility after p_open_to_meeting, keeping all existing logic intact.
...
-- Grant execute to authenticated users with the updated 14-param signature
